...
def login_Doctor(request):    
    if request.method == "POST":        
        m = sqlc.connect(host="localhost",user="vijay",passwd="DummyPassword",database='HMS')
        cursor = m.cursor()
        d = request.POST
...
def doctors(request,*args,**kwargs):
    ...
    # f.close
    # print(usr)
    m = sqlc.connect(host="127.0.0.1", user="vijay", passwd="DummyPassword", database='HMS')
    cursor = m.cursor()
    ...
    query1 = "SELECT PatientID,Name,Gender,Age,Diagnosis FROM Patient LEFT JOIN Appointment ON Patient.PatientID = Appointment.Patient LEFT JOIN Prescribes_Medication ON Patient.PatientID = Prescribes_Medication.Patient LEFT JOIN Prescribes_Treatment ON Patient.PatientID = Prescribes_Treatment.Patient LEFT JOIN Prescribes_Test ON Patient.PatientID = Prescribes_Test.Patient WHERE Appointment.Doctor = '{}';".format(user)
    ...
    if request.method == "POST":
        ...
        for key, value in d.items():
            ...
            if key == "appointmentid":
                appointmentid = value
        if pid_med == 0:
            query2 = "SELECT DISTINCT * FROM Patient LEFT JOIN Appointment ON Patient.PatientID = Appointment.Patient LEFT JOIN Prescribes_Medication ON Patient.PatientID = Prescribes_Medication.Patient LEFT JOIN Prescribes_Treatment ON Patient.PatientID = Prescribes_Treatment.Patient LEFT JOIN Prescribes_Test ON Patient.PatientID = Prescribes_Test.Patient WHERE Appointment.Doctor = '{}' AND Appointment.Patient = '{}';".format(user,pid)
